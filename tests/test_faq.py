...
def test_faq_answer_text(main_page, question_index, expected_answer):
    ...
    with allure.step("Дождаться загрузки и закрыть баннер cookies"):
        allure.attach("Ждём появления ключевых элементов и закрываем баннер cookies, если он отображается.",
